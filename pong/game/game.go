...
func Build(level *ldtkgo.Level, gc GameConfig, lc LevelConfig) *LoadedLevel {
	...
	for _, ent := range level.EntitiesByTag("player") {
		...
		archetype.NewPaddle(e.World, ent, side, lc.PaddleSpeed)
	}

	// Spawn ball.
	for _, ent := range level.EntitiesByTag("ball") {
		archetype.NewBall(e.World, ent, lc.BallSpeed)
	}

	// Spawn walls from IntGrid.
	if ig := level.IntGrid("collision_test"); ig != nil {
		archetype.NewWallsFromIntGrid(e.World, ig)
	}
...
func MergeLevelConfig(defaultCfg, override LevelConfig) LevelConfig {
	merged := LevelConfig{
		BallSpeed:   defaultCfg.BallSpeed,
		PaddleSpeed: defaultCfg.PaddleSpeed,
	}
	if override.BallSpeed != 0 {
		merged.BallSpeed = override.BallSpeed
	}
	if override.PaddleSpeed != 0 {
		merged.PaddleSpeed = override.PaddleSpeed
	}
