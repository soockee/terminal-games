		...
		case ldtkgo.LayerTypeTile:
			...
			if tiles := layer.AllTiles(); len(tiles) > 0 {
				...
				for _, tileData := range tiles {
					...
					// Subimage the Tile from the Tileset
					tileImage := er.Tilesets[er.CurrentTileset].SubImage(image.Rect(tileData.Src[0], tileData.Src[1], tileData.Src[0]+layer.GridSize, tileData.Src[1]+layer.GridSize))
					tile := tileImage.(*ebiten.Image)
